...
export default function CrButton({
  ...
  return (
    <button type={type} className={buttonClass} disabled={disabled} onClick={onClick}>
      {leftIcon && <span className="cr-btn__icon cr-btn__icon--left">{leftIcon}</span>}
      <span className="cr-btn__text">{children}</span>
      {rightIcon && <span className="cr-btn__icon cr-btn__icon--right">{rightIcon}</span>}
    </button>
  )
}
