// CrDjScheduleSearch.stories.tsx
...
export default {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          'Built from search input and filter elements. DJ schedule search component with text search and filtering capabilities. Allows users to find specific DJs, shows, or time slots in the schedule. Includes clear and submit functionality. Dark mode adapts through [data-theme="dark"] CSS selectors.',
      },
    },
  },
  argTypes: {
...
export const NoResults = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'Search with no matching results shows "Sorry, there are no DJs or Shows by that name" message.',
      },
    },
  },
}
  ...
  render: () => {
    ...
    const handleSearch = (searchQuery) => {
      ...
      const filtered = mockSearchResults.filter((show) => {
        const djMatch = show.dj.some((dj) => dj.toLowerCase().includes(searchQuery.toLowerCase()))
        const titleMatch =
          show.title && show.title.toLowerCase().includes(searchQuery.toLowerCase())
