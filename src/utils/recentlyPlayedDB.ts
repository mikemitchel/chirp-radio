...
export function getRecentlyPlayed(): RecentlyPlayedTrack[] {
  try {
    ...
    return tracks.sort(
      (a: RecentlyPlayedTrack, b: RecentlyPlayedTrack) =>
...
export function addRecentlyPlayed(track: Omit<RecentlyPlayedTrack, 'playedAt'>): void {
  ...
  window.dispatchEvent(
    new CustomEvent('chirp-recently-played-updated', {
 ...
 */
export function updateRecentlyPlayedAlbumArt(
  artistName: string,
  trackName: string,
  albumArt: string
): void {
  ...
  const trackIndex = tracks.findIndex(
    (t) => t.artistName === artistName && t.trackName === trackName
  )
 ...
 */
export function groupByHour(tracks: RecentlyPlayedTrack[]): Record<
  string,
  {
    hourData: {
      startTime: string
