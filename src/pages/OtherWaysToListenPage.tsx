...
const OtherWaysToListenPage: React.FC = () => {
  ...
  const { data: pageConfig } = usePageBySlug('other-ways-to-listen')
  ...
  // Get the announcement specified in CMS or fallback to index 3
  const selectedAnnouncement = typeof pageConfig?.sidebarAnnouncement === 'object' ? pageConfig.sidebarAnnouncement : typeof pageConfig?.sidebarAnnouncement === 'string' || typeof pageConfig?.sidebarAnnouncement === 'number' ? announcements?.find(a => String(a.id) === String(pageConfig.sidebarAnnouncement)) : announcements?.[3]
  ...
  return (
    <>
      <Helmet>
        <title>{pageConfig?.title || 'Other Ways to Listen | CHIRP Radio'}</title>
        ...
        <div className="page-layout-main-sidebar__main">
          ...
          {contentBlocks.length > 0 ? (
            <>
              ...
              {/* Remaining blocks in 2-column grid */}
              <div className="grid-2col-equal">
                <div>
                  {/* Blocks 1-2 in left column */}
              ...
              {/* Block 6 (final full-width block if present) */}
              {contentBlocks[6] && (
                <CrCard
                  variant="article"
                  type="page"
