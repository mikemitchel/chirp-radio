...
export default function CrDjSchedule({
  ...
  const currentDay = centralTime.toLocaleDateString("en-US", { weekday: "long", timeZone: "America/Chicago" });
  ...
  const handleSearch = (query) => {
    ...
    const results = allShows.filter(show => {
      const djMatch = show.dj && Array.isArray(show.dj) && show.dj.some(dj => 
        dj.toLowerCase().includes(query.toLowerCase())
      );
  ...
  return (
    <div className={`cr-dj-schedule ${className}`}>
      ...
      <div className="cr-dj-schedule__sticky-header">
        ...
        <h2 className="cr-dj-schedule__day-label">
          {activeDay}
        </h2>
      </div>
        ...
        {timeBuckets.map(bucket => (
          <div key={bucket} className="cr-dj-schedule__column">
            ...
            {organizedShows[bucket].map(show => {
              ...
              const start = parseFloat(show.start.split(':')[0]) + 
                           parseFloat(show.start.split(':')[1]) / 60;
              const end = show.end === '00:00' ? 
                         24 : parseFloat(show.end.split(':')[0]) + 
                              parseFloat(show.end.split(':')[1]) / 60;

              const isCurrent = currentDay === activeDay && 
