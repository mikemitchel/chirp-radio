...
export default {
  ...
  parameters: {
    ...
    docs: {
  description: {
    component: 'Built using the CrButton atom, image elements, and CrTrackInfo info molecule with button interactions. Playlist track item showing album art, track details, and time played. This uses track info molecule - consider moving to Organisms category. Supports both default card and table row layouts. Dark mode adapts through [data-theme="dark"] CSS selectors.'
  }
}
  },
  argTypes: {
...
export const TablePlaylistExample = {
  render: () => {
    ...
    return React.createElement('div', {
      ...
      React.createElement('div', {
        ...
        React.createElement('div', { key: 'art', style: { width: '45px', flexShrink: 0 } }),
        React.createElement('div', { key: 'grid', style: { display: 'flex', flex: 1, gap: 'var(--cr-space-8)' } }, [
          React.createElement('div', { key: 'left', style: { flex: 1 } }, 'Artist / Track'),
          React.createElement('div', { key: 'right', style: { flex: 1 } }, 'Album / Label')
        ]),
        React.createElement('div', { key: 'time', style: { width: '80px', textAlign: 'right', flexShrink: 0 } }, 'Time'),
        React.createElement('div', { key: 'action', style: { width: '70px', flexShrink: 0 } })
      ]),
      // Items
  ...
  parameters: {
    docs: {
      description: {
        story: 'Multiple playlist items displayed in table row format with header, showing a more compact layout suitable for playlists and history views.'
      }
    }
  }
};
